...
public class BallDropServer extends PApplet {
...
float ballSize = 10.0f;             // the size of the ball
float ballDirectionV = 0.0f;        // the ball's horizontal direction. 
// left is negative, right is positive
float ballDirectionH = 1.5f;        // the ball's vertical direction.
...
int hitDeck = 0;
float paddleHeight = 10.0f;     // vertical dimension of the paddles
float paddleWidth = 90.0f;      // horizontal dimension of the paddles
...
public void gameSetup() {
  ...
  nextPaddleV = height - ((paddleHeight * 2.0f) * 16.0f);
  highestPaddleV = nextPaddleV;

  nextPaddleH = paddleWidth;
  // initialize the ball in the center of the screen:
  ballPosV = ballSize * 2.0f;
  ballPosH = PApplet.parseInt(random(width));
...
public void makeNewPlayer(Client thisClient) {
  ...
  There a number of things it might have said that we care about:
    ...
    if (speakingPlayer.gettingName == true) {
      ...
      case '\r': // ignore the carriage return
        break;
      default:
        speakingPlayer.name += PApplet.parseChar(whatClientSaid);
      }
    } else {
      switch (whatClientSaid) { 
...
public void moveBall() {
  ...
  for (int p = 0; p < playerList.size (); p++) {
    ...
    float paddleRight = thisPlayer.paddleH + paddleWidth/2.0f;
    float paddleLeft = thisPlayer.paddleH - paddleWidth/2.0f;
    ...
    if ((ballPosH >= paddleLeft) && (ballPosH <= paddleRight)) {
      ...
      float paddleTop = thisPlayer.paddleV - paddleHeight/2.0f;
      float paddleBottom = thisPlayer.paddleV + paddleHeight/2.0f;

      // check the new vertical position against the paddle vertical position:
      if (ballDirectionV >= 0.0f) {
        // ball is descending
        if ((newBallPosV >= paddleTop) && (ballPosV < paddleTop)) {
          ...
          if (lastPlayerHit != thisPlayer) {
            ...
            ballDirectionH = constrain(ballDirectionH + (thisPlayer.paddleVelocity/10.0f), 0.1f, 2.0f);
          }
        }
  ...
  if (newBallPosV > height) {
    newBallPosV = height - ballSize/2.0f;
    collided = true;
  }
  ...
  if ((ballPosH - ballSize/2.0f <= 0.0f) || (ballPosH +ballSize/2.0f >=width)) {
  ...
  if ((abs(ballDirectionV) <= 0.3f) && abs(height - newBallPosV ) <= 10.0f) {
    ballDirectionV = 0.0f; 
    resetBall();
  }
}
...
public class Player {  
  ...
  public Player (float hpos, float vpos, Client someClient) {
    ...
    int[] octets = PApplet.parseInt(split(someClient.ip(), "."));
    if (octets.length > 2) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "BallDropServer" };
    if (passedArgs != null) {
